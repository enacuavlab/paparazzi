...
def getpos(event,x,y,flags,param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        if mouse_hsv is None:
            ...
            cv2.setTrackbarPos('v_min', 'hist', v_min)
            cv2.setTrackbarPos('v_max', 'hist', v_max)
            mouse_hsv = None
            updated = True
...
cv2.createTrackbar('v_min', 'hist', v_min, 255, set_v_min)
cv2.createTrackbar('v_max', 'hist', v_max, 255, set_v_max)
...
def plot_hist():
    ...
    mask = None
    if h_min <= h_max:
        lower = np.array([h_min, _s_min, v_min])
        upper = np.array([h_max, _s_max, v_max])
        mask = cv2.inRange(hsv, lower, upper)
    else: # splitted
        lower1 = np.array([0, _s_min, v_min])
        upper1 = np.array([h_max, _s_max, v_max])
        mask1 = cv2.inRange(hsv, lower1, upper1)
        lower2 = np.array([h_min, _s_min, v_min])
        upper2 = np.array([179, _s_max, v_max])
    ...
    # HSV output
    print("[[{}, {}, {}],[{}, {}, {}]] | {} {} {} {} {} {}".format(
        h_min, min(s_min,s_max), v_min,
        h_max, max(s_min,s_max), v_max,
        h_min, min(s_min,s_max), v_min,
        h_max, max(s_min,s_max), v_max))
