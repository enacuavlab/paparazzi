...
static void bmp280_register_write(struct bmp280_t *bmp, uint16_t reg, uint8_t value);
...
void bmp280_init(struct bmp280_t *bmp)
{
  ...
  if(bmp->bus == BMP280_SPI) {
    ...
    bmp->spi.trans.output_buf = inv->spi.tx_buf;
    bmp->spi.trans.status = SPITransDone;

    bmp->rx_buffer = bmp->spi.rx_buf;
  ...
  /* I2C setup */
    ...
    bmp->i2c.i2c_trans.slave_addr = bmp->i2c.slave_addr; // slave address
    bmp->i2c.i2c_trans.status = I2CTransDone;  // set initial status: Done
     ...
     (bmp->bus == BMP280_I2C && bmp->i2c.trans.status == I2CTransDone)) {
      ...
      switch (bmp->status) {
        ...
        case BMP280_STATUS_CONFIGURE:
          ...
          bmp280_register_write(bmp, BMP280_CTRL_MEAS_REG_ADDR, (BMP280_OVERSAMPLING_1X_T | BMP280_OVERSAMPLING_8X_P | BMP280_POWER_NORMAL_MODE));
          bmp280_register_write(bmp, BMP280_CONFIG_REG_ADDR, (BMP280_INACTIVITY_62_5_MS | BMP280_IIR_FILTER_COEFF_2));
...
{
  /* Successful transfer */
  if((inv->bus == BMP280_SPI && inv->spi.trans.status == SPITransSuccess) || 
     (inv->bus == BMP280_I2C && inv->i2c.trans.status == I2CTransSuccess)) {
      switch (bmp->status) {

        case BMP280_STATUS_GET_CALIB:
          // compute calib
          parse_calib_data(bmp, &bmp->rx_buffer[0]); //TODO
...
static void parse_sensor_data(struct bmp280_t *bmp, uint8_t *data)
{
  ...
  data_msb = (uint32_t)bmp->data[0] << 16;
  data_lsb = (uint32_t)bmp->data[1] << 8;
  data_xlsb = (uint32_t)bmp->data[2];
  bmp->raw_pressure = (int32_t)((data_msb | data_lsb | data_xlsb) >> 4);

  /* Store the parsed register values for temperature data */
  data_msb = (uint32_t)bmp->data[3] << 16;
  data_lsb = (uint32_t)bmp->data[4] << 8;
  data_xlsb = (uint32_t)bmp->data[5];
...
static void bmp280_register_read(struct bmp280_t *bmp, uint8_t reg, uint16_t size) {
  ...
  if(bmp->bus ==   BMP280_SPI) {
    bmp->spi.trans.output_length = 2;
    bmp->spi.trans.input_length = 1 + size;
    bmp->tx_buffer[1] = 0;
    spi_submit(bmp->spi.p, &(bmp->spi.trans));
  }
  /* I2C transaction */
  else {
    i2c_transceive(bmp->i2c.p, &(bmp->i2c.trans), bmp->i2c.slave_addr, 1, (1 + size));
  }
}