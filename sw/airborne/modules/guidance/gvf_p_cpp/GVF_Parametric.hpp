...
class GVF_Parametric
{
  ...
  full_output compute_step(float x, float y, float z, float vx, float vy, float vz);

  // Compute the values of the GVF Parametric field with curve normalization (adaptive stepping enabled)
  full_output compute_normalized_step(float x, float y, float z, float vx, float vy, float vz);
};
