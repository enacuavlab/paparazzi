...
struct FloatControlInput control_increment_FA;  //[dtheta, dphi, dTx,dTy,dTz]
//
...
struct ThrustSetpoint thrust_sp;     // WHY it is not thrust (THRUST_INCR_SP)
float thrust_in;
...
// By MH ( Should define T before)
static void guidance_indi_calcG_yxz_FA(struct FloatMat35 *Gmat, struct FloatEulers *euler_yxz, struct FloatVect3 *T) 
...
{
  FLOAT_EULERS_ZERO(guidance_euler_cmd); // where it is defined initially
  THRUST_SP_SET_ZERO(thrust_sp);        // where it is defined initially
 ...
 */
void guidance_indi_calcG_yxz_FA(struct FloatMat35 *Gmat, struct FloatEulers *euler_yxz, struct FloatVect3 *T) // By MH
{
  // Euler Angles
  float sphi = sinf(euler_yxz->phi);
  float cphi = cosf(euler_yxz->phi);
