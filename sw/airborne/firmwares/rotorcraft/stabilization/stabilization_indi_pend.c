...
#ifdef STABILIZATION_INDI_FILT_CUTOFF_PEND
#define STABILIZATION_INDI_FILTER_PEND_RATE TRUE
#else
#define STABILIZATION_INDI_FILT_CUTOFF_PEND 20.0
#endif
...
float indi_pend_gain_att = STABILIZATION_INDI_REF_ERR_PEND; 
float indi_pend_gain_rate = STABILIZATION_INDI_REF_RATE_PEND;
...
float angular_accel_ref_pend = 0.;
struct FloatRates angular_rate_ref = {0., 0., 0.};
float angular_acceleration[4] = {0., 0., 0., 0.};
...
Butterworth2LowPass measurement_lowpass_filters[4];
Butterworth2LowPass estimation_output_lowpass_filters[4];
Butterworth2LowPass acceleration_lowpass_filter;
#if STABILIZATION_INDI_FILTER_RATES_SECOND_ORDER
Butterworth2LowPass rates_filt_so[4];
#else
static struct FirstOrderLowPass rates_filt_fo[4];
#endif
...
void init_filters(void)
{
  ...
  tau = 1.0 / (2.0 * M_PI * STABILIZATION_INDI_FILT_CUTOFF_PEND);
  init_butterworth_2_low_pass(&rates_filt_so[3], tau, sample_time, 0.0);
#else
  // Init rate filter for feedback
  float time_constants[4] = {1.0 / (2 * M_PI * STABILIZATION_INDI_FILT_CUTOFF_P), 1.0 / (2 * M_PI * STABILIZATION_INDI_FILT_CUTOFF_Q), 1.0 / (2 * M_PI * STABILIZATION_INDI_FILT_CUTOFF_R), 1.0 / (2 * M_PI * STABILIZATION_INDI_FILT_CUTOFF_PEND)};
  ...
  init_first_order_low_pass(&rates_filt_fo[3], time_constants[2], sample_time, stateGetBodyRates_f()->r);
#endif
}
 ...
 */
void stabilization_indi_rate_run(struct FloatRates rate_sp, float rate_pend_sp, bool in_flight)
{
  ...
  //pendulum estimation
  update_butterworth_2_low_pass(&measurement_lowpass_filters[3], -gyro_imu_f.q);
  update_butterworth_2_low_pass(&estimation_output_lowpass_filters[3], -gyro_imu_f.q);
  //Calculate the angular acceleration via finite difference
  angular_acceleration[3] = (measurement_lowpass_filters[3].o[0]
                               - measurement_lowpass_filters[3].o[1]) * PERIODIC_FREQUENCY;
  // Calculate derivatives for estimation
  // float estimation_rate_d_prev = estimation_rate_d[3];
  // estimation_rate_d[3] = (estimation_output_lowpass_filters[3].o[0] - estimation_output_lowpass_filters[3].o[1]) *
  //                         PERIODIC_FREQUENCY;
  // estimation_rate_dd[3] = (estimation_rate_d[3] - estimation_rate_d_prev) * PERIODIC_FREQUENCY;
...
#if STABILIZATION_INDI_FILTER_PEND_RATE
#if STABILIZATION_INDI_FILTER_RATES_SECOND_ORDER
  rates_pend_filt = update_butterworth_2_low_pass(&rates_filt_so[3], -gyro_imu_f.q);
#else
  rates_pend_filt = update_first_order_low_pass(&rates_filt_fo[3], -gyro_imu_f.q);
#endif
#else
  rates_pend_filt = -gyro_imu_f.q;
#endif
  ...
  angular_accel_ref_pend = (rate_pend_sp - rates_pend_filt) * indi_pend_gain_rate;
...
// #endif
  ...
  indi_v[3] = v_thrust.z;
  indi_v[4] = (angular_accel_ref_pend - use_increment * angular_acceleration[3]);;
...
void stabilization_indi_attitude_run(struct Int32Quat quat_sp, bool in_flight)
{
  ...
  float angle_wing2fus = -encoder_amt22.H_g_filter.hatx[0];
  QUAT_ASSIGN(quat_wing2fus, cos(angle_wing2fus/2), 0.0, sin(angle_wing2fus/2), 0.0);
  float_quat_comp(&quat_fus, att_quat, &quat_wing2fus);
  ...
  // rate_pend_sp = indi_pend_gain_att * quat_fus.qy / indi_pend_gain_rate;
  rate_pend_sp = indi_pend_gain_att * euler_fus.theta / indi_pend_gain_rate;
  ...
  stabilization_indi_rate_run(rate_sp, rate_pend_sp, in_flight);
