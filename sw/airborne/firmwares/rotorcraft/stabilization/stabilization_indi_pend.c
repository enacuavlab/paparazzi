...
void stabilization_indi_update_filt_freq(float freq)
{
  ...
  init_butterworth_2_low_pass(&rates_filt_so[3], tau, sample_time, -gyro_imu_f.q);
#else
  ...
  init_first_order_low_pass(&rates_filt_fo[3], tau, sample_time, -gyro_imu_f.q);
#endif
}
...
void init_filters(void)
{
  ...
  init_first_order_low_pass(&rates_filt_fo[3], time_constants[3], sample_time, -gyro_imu_f.q);
#endif
}
 ...
 */
void stabilization_indi_rate_run(bool in_flight, struct StabilizationSetpoint *sp, struct ThrustSetpoint *thrust, int32_t *cmd, float rate_pend_sp)
{
  ...
  float_vect_diff(angular_acc_disturbance_estimate, angular_acceleration, angular_acc_prediction_filt, 4);
  ...
  indi_v[3] = v_thrust.z;
  indi_v[4] = (angular_accel_ref_pend - angular_acc_disturbance_estimate[3]);
...
void stabilization_indi_attitude_run(bool in_flight, struct StabilizationSetpoint *att_sp, struct ThrustSetpoint *thrust, int32_t *cmd)
{
  ...
  stabilization_indi_rate_run(in_flight, &sp, thrust, cmd, rate_pend_sp);
}
