...
void stabilization_hover_wind_run(bool in_flight){
 ...
 //ELEVON_LEFT  
  ...
  // <!-- REXT RINT LEXT LINT ELEVON_L ELEVON_R-->
  // actuators_pprz[0] -> REXT, actuators_pprz[1] -> RINT,  actuators_pprz[2] -> LEXT,  actuators_pprz[3] -> LINT,
  // actuators_pprz[4] -> ELEVON_L,  actuators_pprz[5] -> ELEVON_R
  actuators_pprz[1]=TRIM_UPPRZ(u_scale[1][0]);
  actuators_pprz[2]=TRIM_UPPRZ(u_scale[1][0]);
  actuators_pprz[3]=TRIM_UPPRZ(u_scale[0][0]); 
  actuators_pprz[4]=TRIM_UPPRZ(u_scale[0][0]); 

  actuators_pprz[4]=TRIM_PPRZ(u_scale[2][0]); 
  actuators_pprz[5]=TRIM_PPRZ(u_scale[3][0]);  


  
  
  if (in_flight) {
    stabilization_cmd[COMMAND_THRUST] = (actuators_pprz[1]+actuators_pprz[2]+actuators_pprz[3]+actuators_pprz[4]); // for in_flight detection
    //printf("in_flight\n");
  } else {
